...
object Cats {
  ...
  def increment[F[_]](container: F[Int])(using functur:Functor[F]):F[Int] = functur.map(container)(_+1)
  import cats.syntax.functor.*
  def increment_v2[F[_]](container: F[Int])(using functur:Functor[F]):F[Int] = container.map(_+1)
