...
//   need given ConfigReader[Host] + given ConfigReader[Port] => compiler generate ConfigReader[EmberConfig]
  implicit val hostReader: ConfigReader[Host] = ConfigReader[String].emap( hostString => Host.fromString(hostString).toRight(CannotConvert(hostString, Host.getClass.toString, s"Invalid host name $hostString")) )
